...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(
        ...
        return self._get(record, frame_start_indices)  # frame_start_indices: [0.0, 16.0, 32.0, 48.0, 64.0, 80.0, 96.0, 112.0, 128.0, 144.0, 160.0, 176.0, 192.0, 208.0, ..., 496]
...
if __name__ == '__main__':
    ...
    features, labels, label, segment_size = test_data.__getitem__(0)
    print(features.shape)  # torch.Size([1, 1536, 512])  代表 [ batch，长度（包括扩展后的长度）, dimension ]  扩展长度=（（原长度）// 512）+ 1）* 512
    print(labels.shape)  # (1412,) 每帧的标签：[7,7,...,0,0,0,...,7,7,7] 
    print(label) # 0 视频级别的标签
    print(segment_size) # 视频长度为512的倍数

    print(len(train_data_normal))
    print(len(train_data_anomaly))
    print(len(test_data))
    ...
    # a = train_data_normal.__getitem__(1)
    # print(a[0].shape)
    # print(a[1])
    # print(ninput.shape)
